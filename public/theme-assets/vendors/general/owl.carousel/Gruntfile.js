		...
		.initConfig({
			...
			assemble: {
				options: {
					...
					expand: true,
					production: false,
					assets: '<%= app.docs.dest %>/theme-assets',
			...
			sass: {
				docs: {
					options: {
						...
						includePaths: [ '<%= app.docs.src %>/theme-assets/scss/', 'node_modules/foundation-sites/scss' ]
					},
					files: {
						'<%= app.docs.dest %>/assets/css/docs.theme.min.css': '<%= app.docs.src %>/theme-assets/scss/docs.theme.scss'
					}
				},
				dist: {
					options: {
			...
			autoprefixer: {
				...
				dist: {
					files: {
						'dist/assets/<%= pkg.name %>.css': 'dist/theme-assets/<%= pkg.name %>.css',
						'dist/assets/owl.theme.default.css': 'dist/theme-assets/owl.theme.default.css',
						'dist/assets/owl.theme.green.css': 'dist/theme-assets/owl.theme.green.css'
					}
				}
			},
			...
			cssmin: {
				dist: {
					files: {
						'dist/assets/<%= pkg.name %>.min.css': 'dist/theme-assets/<%= pkg.name %>.css',
						'dist/assets/owl.theme.default.min.css': 'dist/theme-assets/owl.theme.default.css',
						'dist/assets/owl.theme.green.min.css': 'dist/theme-assets/owl.theme.green.css'
					}
				}
			},
			...
			copy: {
				...
				distToDocs: {
					...
					cwd: 'dist/',
					src: [ '**/*.*' ],
					dest: '<%= app.docs.dest %>/theme-assets/owlcarousel'
				},
				...
				srcToDocs: {
					...
					cwd: 'src/js',
					src: [ '**/*.js' ],
					dest: '<%= app.docs.dest %>/theme-assets/owlcarousel/src'
				},

				docsAssets: {
					expand: true,
					cwd: '<%= app.docs.src %>/theme-assets/',
					src: [ 'css/*.css', 'vendors/*.js', 'vendors/*.map', 'img/*.*', 'js/*.*' ],
					dest: '<%= app.docs.dest %>/theme-assets/'
				},
			...
			watch: {
				...
				sassDocs: {
					files: [ '<%= app.docs.src %>/theme-assets/**/*.scss' ],
					tasks: [ 'sass:docs' ]
				},
				sass: {
					files: [ 'src/**/*.scss' ],
				...
				jsDocs: {
					files: [ '<%= app.docs.src %>/theme-assets/**/*.js' ],
